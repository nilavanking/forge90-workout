...
(async()=>{
  ...
  try{
    ...
    const click=async(kind,action,index='')=>page.locator(`[data-c-action="${action}"][data-kind="${kind}"][data-index="${index}"]`).click();
    ...
    await page.locator('[data-c-load]').fill('20');await page.locator('[data-c-load]').dispatchEvent('change');
    ...
    await click('warmup','plus','1');assert.equal((await state()).warmup[1].targetMs,120000);await click('warmup','minus','1');assert.equal((await state()).warmup[1].targetMs,60000);
    await click('warmup','skip','1');await page.locator('#f90c-reason-dialog [name=reason]').selectOption('Time shortage');await page.getByRole('button',{name:'Confirm skip',exact:true}).click();assert.equal((await state()).warmup[1].status,'Skipped');pass('individual warm-up adjustments and structured skip reason');
    ...
    const first=page.locator('#exerciseCards .set-row').first();await first.locator('input[type=number]').nth(0).fill('40');await first.locator('input[type=number]').nth(1).fill('10');
    ...
    const addon=page.locator('#forge90-gym-addons .f90v2-start').first();await addon.click();await page.clock.fastForward(15000);await addon.click();await page.click('[data-f90=skip]');assert.ok((await page.evaluate(()=>window.Forge90Conditioning.getSummary())).coreMs>=15000);pass('total session pause excludes pause time and core work has separate duration');
    ...
    await click('cardio','start');await page.clock.fastForward(60000);await page.reload({waitUntil:'networkidle'});await page.click('#startWorkoutBtn');await page.waitForSelector('#f90c-cardio');assert.equal((await state()).cardio.status,'Running');assert.ok((await page.evaluate(()=>window.Forge90Conditioning.getSummary())).cardioMs>=60000);pass('running session survives refresh and Start Workout resumes it');
    ...
    await page.clock.fastForward(16*60000);assert.equal((await page.evaluate(()=>window.Forge90ConditioningCore.phase(window.Forge90Conditioning.getSession().cardio,Date.now()))),'Cooldown');await page.clock.fastForward(4*60000);assert.equal((await state()).cardio.status,'Completed');
    ...
    await page.getByText('Optional completion alerts',{exact:true}).click();
    ...
    await page.click('#resetWorkoutBtn');await page.waitForTimeout(200);await click('cardio','minus');await click('cardio','minus');await click('cardio','minus');assert.equal((await state()).cardio.targetMs,5*60000);assert.equal(await page.locator('[data-c-action=minus][data-kind=cardio]').isDisabled(),true);await click('cardio','skip');await page.locator('#f90c-reason-dialog [name=reason]').selectOption('Machine unavailable');await page.getByRole('button',{name:'Confirm skip',exact:true}).click();assert.equal((await state()).cardio.status,'Skipped');assert.equal((await state()).cardio.elapsedMs,0);pass('cardio minimum target and Skipped remain distinct from Completed');
