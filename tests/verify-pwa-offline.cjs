...
(async () => {
  ...
  const profile=fs.mkdtempSync(path.join(root,'test-results','pwa-profile-'));
  ...
  try {
    ...
    const expectedCache=fs.readFileSync(path.join(root,'sw.js'),'utf8').match(/const CACHE='([^']+)'/)[1];
    assert.ok(online.names.includes(expectedCache));
    ...
    const cached = online.entries[expectedCache];
    for (const asset of ['/', '/index.html', '/styles.css', '/app.js', '/vendor/dexie.min.js',
      ...
      '/forge90-enhancements.js', '/forge90-weight.js', '/forge90-conditioning-core.js', '/forge90-measurements.js', '/forge90-progression-core.js', '/forge90-conditioning.js', '/manifest.webmanifest',
